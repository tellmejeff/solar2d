...
module.init = function(physics, group, sheet, state, fireSound)
    module.physics = physics
    module.group = group
    module.sheet = sheet
    module.state = state
    module.fireSound = fireSound
end
...
module.fireLaser = function(offset)
    if (not isEmpty(module.ship) and not isEmpty(module.ship.x)) then
        ...
        newLaser:toBack()
        transition.to(newLaser, { x = newLaser.x + offset * 5, y = -40 - (display.contentHeight - module.ship.y), time = 900,
                                  onComplete = function()
